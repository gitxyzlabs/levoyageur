...
import { Award, Users, Star, ChevronLeft, ChevronRight, MapPin, Edit3, Navigation, Heart, Bookmark, Calendar, X, ExternalLink, Share2 } from 'lucide-react';
...
export function MobileInfoSheet({ 
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `Check out ${place.name || 'this place'} on Le Voyageur`,
      url: shareUrl
    };
    ...
    if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
      try {
        ...
        if (error.name !== 'AbortError') {
          ...
          fallbackShareToClipboard(shareUrl);
        }
      }
    } else {
      // Fallback to clipboard
  ...
  const fallbackShareToClipboard = async (url: string) => {
    try {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              {place.rating && (
                ...
                >
                  ...
                  <div className="text-3xl font-bold text-slate-900 group-hover:text-blue-700 transition-colors">{place.rating.toFixed(1)}</div>
                </button>
              )}
            </div>
            ...
            <button
              onClick={handleShare}
              className="w-full flex items-center justify-center gap-2 py-4 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium transition-all border border-gray-300"
            >
