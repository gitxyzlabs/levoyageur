...
export default function MapView() {
  ...
  useEffect(() => {
    const handleSharedPlace = async () => {
      ...
      trackAction('shared_place_opened', 'MapView', { placeId });
      ...
      try {
        ...
        if (coordMatch) {
          ...
          if (nearbyLocation) {
            const place: google.maps.places.PlaceResult = {
              place_id: nearbyLocation.place_id || nearbyLocation.googlePlaceId,
              name: nearbyLocation.name,
              formatted_address: nearbyLocation.description,
              geometry: {
                location: { lat, lng } as google.maps.LatLng
              },
              rating: nearbyLocation.googleRating ?? undefined,
            };

            setSelectedGooglePlace(place);
            setSelectedLVLocation(nearbyLocation);
            toast.success(`Opened ${nearbyLocation.name}`);
          } else {
        ...
        if (lvLocation && lvLocation.lat && lvLocation.lng) {
          // We have the location in our database
          ...
          // Create a place object for the info window
          const place: google.maps.places.PlaceResult = {
    ...
    handleSharedPlace();
  }, [placeId, googleMapsApiKey, locations, navigate]);
          ...
          <APIProvider apiKey={googleMapsApiKey}>
            ...
            <Map
              ...
              onGooglePlaceClose={() => {
                ...
                setSelectedLVLocation(null);
                setSelectedCity(null);
                // If we're on a shared place URL, navigate back to home
                if (placeId) {
                  navigate('/');
                }
              }}
